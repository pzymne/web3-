...
const scrollAuto = (pEle, cEle, codeMirrorUt) => {
  ...
  const getTopByLine = (line) => {
    return view2.lineBlockAt(view2.state.doc.line(line + 1).from).top;
  };
  const getBottomByLine = (line) => {
    return view2.lineBlockAt(view2.state.doc.line(line + 1).from).bottom;
  };
  let blockMap = [];
  ...
  const pEleHandler = () => {
    ...
    if (endBottom >= pMaxScrollLength || endElePos > cMaxScrollLength) {
      ...
      const _startEle = document.querySelector(`[data-line="${lineNumer}"]`);
  ...
  const cEleHandler = () => {
    ...
    const endLineScrollTop = getTopByLine(
      end + 1 === view2.state.doc.lines ? end : end + 1
    );
    let blockHeight = 0;
